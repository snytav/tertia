...
long Mesh::Save(int isave)
{
   ...
   for (nprocessor=0; nprocessor<nPEs; nprocessor++) {
      ...
      if (nPE == 0) {
         ...
         fdataset = H5Dcreate(file, Ex, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Ey, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         /* Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Ez, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Bx, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, By, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Bz, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jx, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jy, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jz, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Dens, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      ...
      for (int isort=0; isort<nsorts; isort++) {
         ...
         if (nPE == 0) {
            // Creating the dataset within the dataspace                      */
            fdataset = H5Dcreate(file, DensSort, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
            assert (fdataset >= 0);
            status = H5Dclose(fdataset);
            assert (status >= 0);
         }
   ...
   for (nprocessor=0; nprocessor<nPEs; nprocessor+=nParallelStreams) {
      ...
      assert (file >= 0);
      domain()->out_Flog << "SAVE re-opened the file " << fname << "\n";
      domain()->out_Flog.flush();

      idataset = H5Dcreate(file, idataname, H5T_STD_I64LE, idataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      assert (idataset >= 0);
      ...
      domain()->out_Flog << "SAVE int dataset Nparticles created" << "\n";
      domain()->out_Flog.flush();
      ...
      if (npart[0] > 0) {
         ...
         particles_dataset[0] = H5Dcreate(file, pdataname, electron_tid, particles_dataspace[0], H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert (particles_dataset[0] >= 0);
	 domain()->out_Flog << "SAVE dataset for sort " << 0 << "created \n";
	 domain()->out_Flog.flush();
      }
      ...
      for (is=1; is<nsorts; is++) {
         ...
         particles_dataset[is] = H5Dcreate(file, sortname, ion_tid, particles_dataspace[is], H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
         assert(particles_dataspace[is] >= 0);
	 domain()->out_Flog << "SAVE create dataset for sort " << is << "created \n";
	 domain()->out_Flog.flush();
      }
      ...
      domain()->out_Flog << "SAVE particles assigned \n";
      domain()->out_Flog.flush();

      is = 0;
      if (npart[0] > 0) {
	status = H5Dwrite(particles_dataset[is], electron_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, e_array);
	assert (status >= 0);
	delete[] e_array;
	domain()->out_Flog << "SAVE electrons written \n";
	domain()->out_Flog.flush();
      }

      for (is=1; is<nsorts; is++) {
	if (npart[is] > 0) {
	  status = H5Dwrite(particles_dataset[is], ion_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, pi_array[is]);
	  assert (status >= 0);
	  domain()->out_Flog << "SAVE ions of sort " << is << "written \n";
	  domain()->out_Flog.flush();
	}
      }
