   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) { 
            GetCell(iLayer,j,k).f_Fields[idim] = GetCell(iLayer+1,j,k).f_Fields[idim]; // f1
         }
      }
   }
   int iFullStep = 0;
   MoveParticlesLayer(iLayer, iFullStep, 0.5);  // x2=xn+dx1/2. 
   for (int k=-1; k<l_Mz+1; k++) {
...
//-------------------
   MoveFieldsLayer(iLayer, iFullFieldStep, 0.5); // f2=f(xn+dx1/2.)
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            GetCell(iLayer-1,j,k).f_Fields[idim] = 2.*GetCell(iLayer,j,k).f_Fields[idim]; // 2*f2
         }
      }
   }
   Eenergy = EM_EnergyLayer(iLayer);
   if (Eenergy > 100.) {
      double checke1 = Eenergy;
   };

   iFullStep = 0;
   MoveParticlesLayer(iLayer, iFullStep, 0.5); // x3=xn+dx2/2.
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<CURR_DIM; idim++) {
         GetCell(iLayer,0,k).f_Currents[idim] += GetCell(iLayer,l_My,k).f_Currents[idim];
         GetCell(iLayer,l_My-1,k).f_Currents[idim] += GetCell(iLayer,-1,k).f_Currents[idim];
...
//-------------------
   MoveFieldsLayer(iLayer, iFullFieldStep, 0.5); // f3=f(xn+dx2/2.)
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
         GetCell(iLayer,-1,k).f_Fields[idim] = GetCell(iLayer,l_My-1,k).f_Fields[idim];
         GetCell(iLayer,l_My,k).f_Fields[idim] = GetCell(iLayer,0,k).f_Fields[idim];
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            GetCell(iLayer-1,j,k).f_Fields[idim] += 2.*GetCell(iLayer,j,k).f_Fields[idim]; // 2*f2 + 2*f3
         }
      }
   }
   ...
   iFullStep = 0;
   MoveParticlesLayer(iLayer, iFullStep, 1.); // x4=xn+dx3.
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<CURR_DIM; idim++) {
         GetCell(iLayer,0,k).f_Currents[idim] += GetCell(iLayer,l_My,k).f_Currents[idim];
         GetCell(iLayer,l_My-1,k).f_Currents[idim] += GetCell(iLayer,-1,k).f_Currents[idim];
...
//-------------------
   MoveFieldsLayer(iLayer, iFullFieldStep, 1.); // f4=f(xn+dx3)
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
         GetCell(iLayer,-1,k).f_Fields[idim] = GetCell(iLayer,l_My-1,k).f_Fields[idim];
         GetCell(iLayer,l_My,k).f_Fields[idim] = GetCell(iLayer,0,k).f_Fields[idim];
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            GetCell(iLayer,j,k).f_Fields[idim] += GetCell(iLayer-1,j,k).f_Fields[idim]; // 2*f2 + 2*f3
            GetCell(iLayer,j,k).f_Fields[idim] += GetCell(iLayer+1,j,k).f_Fields[idim]; // f1
            GetCell(iLayer,j,k).f_Fields[idim] *= 1.6; // 1/6*(f1 + 2*f2 + 2*f3 + f4)
            GetCell(iLayer-1,j,k).f_Fields[idim] = 0.; //
         }
      }
   }
 ...
 //-------------------------------------------
   iFullStep = 1;
   MoveParticlesLayer(iLayer, iFullStep, 1.); // xn+1.
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<CURR_DIM; idim++) {
         GetCell(iLayer,0,k).f_Currents[idim] += GetCell(iLayer,l_My,k).f_Currents[idim];
         GetCell(iLayer,l_My-1,k).f_Currents[idim] += GetCell(iLayer,-1,k).f_Currents[idim];
...
//-------------------
   MoveFieldsLayer(iLayer, iFullFieldStep, 1.); 
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
         GetCell(iLayer,-1,k).f_Fields[idim] = GetCell(iLayer,l_My-1,k).f_Fields[idim];
         GetCell(iLayer,l_My,k).f_Fields[idim] = GetCell(iLayer,0,k).f_Fields[idim];
