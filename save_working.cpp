                            ...
                            int iPE, int jPE, int kPE, int nPE)
{
   ...
   domain()->out_Flog << "SAVE field " << SetName << " ";
   domain()->out_Flog.flush();
   double *field1d = new double[l_Mx*Xpartition];
   ...
   if (nPE == 0) {
	...
	for (int k=0; k<l_Mz; k++) {
	  ...
	  for (int jp=0; jp<Ypartition; jp++) {
	    for (int j=0; j<l_My; j++) { 
	      int jl = j + l_My*jp;
	      long atNlarge = l_Mx*Xpartition*(jl + l_My*Ypartition*kl); 
		...
		if (fromN == 0) {
		  ...
		  */
		  //			MPI_Barrier(MPI_COMM_WORLD);
		  if (atN1d > l_Mx*(Xpartition-1) || atN1d < 0) {
		    domain()->out_Flog << "Wrong receive in save! \n";
		    domain()->out_Flog.flush();
		    exit(atN1d);
		  }
		  int ierr = MPI_Recv(&field1d[atN1d], l_Mx, 
				      MPI_DOUBLE,
     ...
     for (int kp=0; kp<Zpartition; kp++) {
	  ...
	  for (int jp=0; jp<Ypartition; jp++) {
		...
		if (nPE == fromN) {
		  ...
		  */
#ifdef V_MPI
		  int ierr = MPI_Bsend(&field[l_Mx*(j+l_My*k)], l_Mx, 
