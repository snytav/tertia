...
void Mesh::GuessFieldsHydroLinLayer(int iLayer)
{
   ...
   for (k=0; k<l_Mz; k++)
   {
      ...
      for (j=0; j<l_My; j++)
      {
         ...
         if (fabs(c.f_JxBeam) > 0 || fabs(c.f_RhoBeam) > 0) {
            double isbeam=1.;
         }
      }
   }
...
//   fftw_execute(planR2R_Jx);
   ...
   for (k=0; k<nz; k++)
   {
      for (j=0; j<ny; j++)
      {
         ...
         long n1 = j + ny*k;
         fft_of_Rho[n1] = fft_of_RhoP[n1] - (fft_of_JxP[n1] - fft_of_Jx[n1]) - aky*hx*fft_of_Jy[n1] - akz*hx*fft_of_Jz[n1];
         fft_of_RhoP[n1] = fft_of_Rho[n1];
         ...
         double rp  = fft_of_RhoP[n1]; // + diff_rp*damp;
//         rp = dens;
         double jxp = fft_of_Jx[n1]; // + diff_jx*damp;
         double jyp = fft_of_Jy[n1]; // + diff_jy*damp;
         double jzp = fft_of_Jz[n1]; // + diff_jz*damp;  
         ...
         fft_of_Jy[n1] = jyp + hx*dens*fft_of_Ey[n1];
         fft_of_Jz[n1] = jzp + hx*dens*fft_of_Ez[n1];
         ...
         fft_of_Jx[n1] = fft_of_JxP[n1] + dens*hx*fft_of_Ex[n1];

/*
         fft_of_Jx[n1] = jxp + hx*dens*fft_of_Ex[n1];
         //         vcRho[n1] = rp + h*dens*vcEx[n1] - aky*h*I*vcJy[n1] - akz*h*I*vcJz[n1];
         fft_of_Rho[n1] = rp + hx*dens*fft_of_Ex[n1] - aky*hx*fft_of_Jy[n1] - akz*hx*fft_of_Jz[n1];
*/
...
//         rp = dens;
         double jx = fft_of_JxP[n1]; //
         double jy = fft_of_Jy[n1]; //
         double jz = fft_of_Jz[n1]; //  

         double rb  = fft_of_RhoBeam[n1];

         double expEy = - diff_jy/dens;
         double expEz = - diff_jz/dens;

         double oldEy = aky/(ak2+dens)*(rp+rb);
         double oldEz = akz/(ak2+dens)*(rp+rb);

         double newEy = (aky*(rp+rb) + diff_jy + dens*fft_of_Ey[n1])/(ak2 + dens);
         fft_of_Ey[n1] = newEy;
         double newEz = (akz*(rp+rb) + diff_jz + dens*fft_of_Ez[n1])/(ak2+dens);
         fft_of_Ez[n1] = newEz;

         double oldEx = fft_of_Ex[n1];
         fft_of_Ex[n1] = -aky/ak2*jy -akz/ak2*jz;

         fft_of_Bx[n1] = -aky/ak2*jz + akz/ak2*jy;
//         fft_of_By[n1] = (-akz*(fft_of_JxP[n1] + fft_of_JxBeam[n1]) + dens*fft_of_Ez[n1])/ak2;
//         fft_of_Bz[n1] =  (aky*(fft_of_JxP[n1] + fft_of_JxBeam[n1]) - dens*fft_of_Ey[n1])/ak2;
         fft_of_By[n1] = (-akz*(jx + fft_of_JxBeam[n1]) - diff_jz  + dens*fft_of_By[n1])/(ak2+dens);
         fft_of_Bz[n1] =  (aky*(jx + fft_of_JxBeam[n1]) + diff_jy + dens*fft_of_Bz[n1])/(ak2+dens);
...
//   cout << "Iterate: maxEx =" << maxEx <<"maxEy =" << maxEy <<"maxEz =" << maxEz << endl;
   ...
   Eerr = max(Eerr, difBx/maxBx); 
   Eerr = max(Eerr, difBy/maxBy); 
   Eerr = max(Eerr, difBz/maxBz); 
