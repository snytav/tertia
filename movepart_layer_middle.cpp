...
void Mesh::MoveLayer(int iLayer) 
{
   ...
   int iFullStep = 0;
   MoveParticlesLayer(iLayer, iFullStep); 
   for (int k=-1; k<l_Mz+1; k++) {
...
//-------------------
   iFullFieldStep = 0;
   MoveFieldsLayer(iLayer, iFullFieldStep);
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            GetCell(iLayer,j,k).f_Fields[idim] += GetCell(iLayer+1,j,k).f_Fields[idim];
            GetCell(iLayer,j,k).f_Fields[idim] *= 0.5;
         }
      }
   }
   */
...
//---Mesh:: ---------------------------------------------->
void Mesh::MoveParticlesLayer(int iLayer, int iFullStep) 
{
   l_Processed = 0;
   f_GammaMax  = 1.;
