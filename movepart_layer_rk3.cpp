...
void Mesh::MoveLayer(int iLayer) 
{
   ...
   int iFullStep = 0;
   MoveParticlesLayer(iLayer, iFullStep, 0.5); 
   for (int k=-1; k<l_Mz+1; k++) {
...
//-------------------
   iFullFieldStep = 1;
   MoveFieldsLayer(iLayer, iFullFieldStep, 0.5);
   for (int k=-1; k<l_Mz+1; k++) {
      for (int idim=0; idim<FLD_DIM; idim++) {
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            ...
            GetCell(iLayer,j,k).f_Fields[idim] *= 2.;
            GetCell(iLayer,j,k).f_Fields[idim] -= GetCell(iLayer+1,j,k).f_Fields[idim];
         }
      }
   }
...
//-------------------
   ...
   for (int k=-1; k<l_Mz+1; k++) {
      for (int j=-1; j<l_My+1; j++) {
         for (int idim=0; idim<FLD_DIM; idim++) {
            GetCell(iLayer,j,k).f_Fields[idim] += GetCell(iLayer+1,j,k).f_Fields[idim];
            GetCell(iLayer,j,k).f_Fields[idim] += 4.*GetCell(iLayer-1,j,k).f_Fields[idim];
            GetCell(iLayer,j,k).f_Fields[idim] *= 1./6.;
         }
      }
   }
