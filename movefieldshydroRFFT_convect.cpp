...
static double *fft_of_Ex, *fft_of_Ey, *fft_of_Ez, *fft_of_Bx, *fft_of_By, *fft_of_Bz, *fft_of_Jx, *fft_of_Jy, *fft_of_Jz, *fft_of_Rho;
...
//      fft_of_Rho[i] = .01;
   ...
   for (i=l_Mx-1; i>=0; i--) {
      ...
      fftw_execute(planR2R_JyBeam);
      fftw_execute(planR2R_JzBeam);
      fftw_execute(planR2R_JyBeamP);
      fftw_execute(planR2R_JzBeamP);
      ...
      for (k=0; k<nz; k++)
      {
         for (j=0; j<ny; j++)
         {
            ...
            double djby = fft_of_JyBeamP[n1]-fft_of_JyBeam[n1];
            double djbz = fft_of_JzBeamP[n1]-fft_of_JzBeam[n1];
//            djby = djbz = 0.;

            fft_of_Ey[n1] = aky/(ak2+dens)*(-rp+rb) + djby/h/(ak2+dens);
            fft_of_Ez[n1] = akz/(ak2+dens)*(-rp+rb) + djbz/h/(ak2+dens);
...
//            djby = djbz = 0.;
            ...
            if (abs(fft_of_EyRho[n1]) < 1*(ak2 + dens)) {
               fft_of_Ey[n1] = fft_of_Ey[n1] - fft_of_EyRho[n1]/(ak2 + dens)*damping;
            };
            if (abs(fft_of_EzRho[n1]) < 1*(ak2 + dens)) {
               fft_of_Ez[n1] = fft_of_Ez[n1] - fft_of_EzRho[n1]/(ak2 + dens)*damping;
            };
      
            fft_of_Ex[n1] = -aky/ak2*(fft_of_Jy[n1] + fft_of_JyBeam[n1]) -akz/ak2*(fft_of_Jz[n1] + fft_of_JzBeam[n1]);
