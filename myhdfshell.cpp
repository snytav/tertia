...
int WriteHDF5RecordInt(hid_t file, char* dataname, int nfields, int *source)
{
   ...
   dataspace = H5Screate_simple(rank1, dimsfi, NULL); 
   assert (dataspace >= 0);
   dataset = H5Dcreate(file, dataname, H5T_STD_I32LE, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
   assert (dataset >= 0);
...
int WriteHDF5RecordLong(hid_t file, char* dataname, int nfields, long *source)
{
   ...
   dataspace = H5Screate_simple(rank1, dimsfi, NULL); 
   assert (dataspace >= 0);
   dataset = H5Dcreate(file, dataname, H5T_STD_I64LE, dataspace, 
...
int WriteHDF5RecordString(hid_t file, char* dataname, int nfields, char *source)
{
   ...
   dataspace = H5Screate_simple(rank1, dimsfi, NULL); 
   assert (dataspace >= 0);
   dataset = H5Dcreate(file, dataname, H5T_STD_U8LE, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
   assert (dataset >= 0);
...
{
   return ReadHDF5Recorddouble(file, dataname, nfields, source);
}
...
int ReadHDF5RecordInt(hid_t file, char* dataname, int nfields, int *source)
{
   ...
   status = H5Dread(dataset, H5T_NATIVE_INT, 
		    H5S_ALL, H5S_ALL, H5P_DEFAULT, source);
   assert (status >= 0);
      // Close the dataspace                                            
   status = H5Sclose(dataspace);
   assert (status >= 0);
...
int ReadHDF5Recorddouble(hid_t file, char* dataname, int nfields, double *source)
{
   ...
   assert (status >= 0);
   assert(dimsfi[0] == nfields);
   status = H5Dread(dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, source);
   assert (status >= 0);
      // Close the dataspace                                            
   status = H5Sclose(dataspace);
   assert (status >= 0);
   ...
   for (int i=0; i<nfields; i++) {
      if (fabs(source[i])>1e10) {
         cout <<"WARNIG ReadHDF5Recorddouble: value at i="<<i<<" is too large: "<<source[i]<<endl;
      };
   };
   return istatus;
}
