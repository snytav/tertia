...
//--- Mesh:: ----------------------->
void Mesh::MoveFieldsLayer(int iIn, int iFullFieldStep)
{
   int i, j, k;
   i = iIn;
   j = l_My/2.;
...
//   I.im = 1.;
   ...
   for (k=0; k<nz; k++)
   {
      ...
      for (j=0; j<ny; j++)
      {
         ...
         rRho[n] = c.f_Dens + c.f_RhoBeam;
         rJx[n] = c.f_Jx + c.f_JxBeam;
         rJy[n] = c.f_Jy + c.f_JyBeam;
         rJz[n] = c.f_Jz + c.f_JzBeam;
   ...
   for (k=0; k<kZmax; k++)
   {
      for (j=0; j<kYmax; j++)
      {
         ...
         if (-1) {
            ...
            vcEy[n1] = vcCoy*vcRho[n1] + vcDensK2*vcEyp[n1];
            vcEz[n1] = vcCoz*vcRho[n1] + vcDensK2*vcEzp[n1];
            vcBy[n1] = -vcCoz*vcJx[n1] + vcDensK2*vcByp[n1];
            vcBz[n1] =  vcCoy*vcJx[n1] + vcDensK2*vcBzp[n1];
         }
