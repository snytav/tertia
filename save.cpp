                            ...
                            int iPE, int jPE, int kPE, int nPE)
{
   ...
   domain()->out_Flog << "SAVE field " << SetName << " ";
   domain()->out_Flog.flush();
   double *field2d = new double[l_My*l_Mx*Xpartition];
   double *field2dsmall = new double[l_My*l_Mx];
   ...
   if (nPE == 0) {
      ...
      dimsfmem[1] = l_Mx*Xpartition;
      dimsfmem[0] = l_My;

      fmemspace = H5Screate_simple(rank2, dimsfmem, NULL); 
      assert (fmemspace >= 0);
               ...
               for (int ip=0; ip<Xpartition; ip++) {
                  ...
                  if (fromN == 0) {
                     for (int j=0; j<l_My; j++) { 
                        for (int i=0; i<l_Mx; i++) {
                           int n = i + l_Mx*j;
                           field2dsmall[n] = field[i+l_Mx*(j+l_My*k)];
                        }
                     }
                  } else {
                     int msgtag = k + kp*l_Mz;
                     ...
                     */
                     //			MPI_Barrier(MPI_COMM_WORLD);
                     int ierr = MPI_Recv(field2dsmall, l_Mx*l_My, 
                        MPI_DOUBLE,
                  ...
                  for (int j=0; j<l_My; j++) { 
                     for (int i=0; i<l_Mx; i++) {
                        int n = i + ip*l_Mx + l_Mx*Xpartition*j;
                        assert(n<l_My*l_Mx*Xpartition);
                        field2d[n] = field2dsmall[i+l_Mx*j];
                     }
                  }
               }
               offset[2] = 0;
            ...
            for (int jp=0; jp<Ypartition; jp++) {
               for (int ip=0; ip<Xpartition; ip++) {
                  ...
                  if (nPE == fromN) {
                     ...
                     */
#ifdef V_MPI
                     int ierr = MPI_Send(field2dsmall, l_Mx*l_My, 
