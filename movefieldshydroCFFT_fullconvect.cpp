...
//            gamma_inv = 1.;
            ...
            double uxpcc = -pcc.f_Jx/ppc.f_Dens;
            double uypcc = -pcc.f_Jy/ppc.f_Dens;
            double uzpcc = -pcc.f_Jz/ppc.f_Dens;
/*
...
*/
      ...
      for (k=0; k<kZmax; k++)
      {
         for (j=0; j<kYmax; j++)
         {
            ...
            complex <double> dJx = h*vcExRho[n1]*damping;
            complex <double> dJy = h*vcEyRho[n1]*damping;
            complex <double> dJz = h*vcEzRho[n1]*damping;

            vcJx[n1] = vcJx[n1] + dJx;
            vcJy[n1] = vcJy[n1] + dJy;
            vcJz[n1] = vcJz[n1] + dJz;

            complex <double> dEx = - aky/ak2*I*dJy - akz/ak2*I*dJz;
            complex <double> dEy = -dJy/(h*(ak2 + dens));
            complex <double> dEz = -dJz/(h*(ak2 + dens));
