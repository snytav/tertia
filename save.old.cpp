...
long Mesh::Save(int isave)
{
   ...
   //=============== saving fields ===============
   float sf_DimFields = Cell::sf_DimFields;
   ...
   if (nPE == 0) {
      ...
      dimsfi[0] = 1;
      written = WriteHDF5RecordFloat(file, "phase", dimsfi[0], &phase);
      assert (written >= 0);
      ...
      written = WriteHDF5RecordFloat(file, "X", dimsfi[0], Xaxis);
      assert (written >= 0);
      ...
      written = WriteHDF5RecordFloat(file, "Y", dimsfi[0], Yaxis);
      assert (written >= 0);
      delete[] Yaxis;
      ...
      written = WriteHDF5RecordFloat(file, "Z", dimsfi[0], Zaxis);
      assert (written >= 0);
      delete[] Zaxis;
   ...
   for (nprocessor=0; nprocessor<nPEs; nprocessor++) {
      ...
      float wc = domain()->GetCntrl()->GetWallClockElapsed();
      float wcm = domain()->GetCntrl()->GetWallClockElapsed();
      float wcp = domain()->GetCntrl()->GetWallClockElapsed();
      ...
      if (nPE == 0) {
         ...
         fdataset = H5Dcreate(file, Ex, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Ex);
      assert (fdataset >= 0);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Ex written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Ey, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Ey);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    // Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Ey written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         /* Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Ez, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Ez);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    // Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Ez written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Bx, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Bx);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Bx written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, By, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, By);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: By written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Bz, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Bz);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Bz written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jx, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Jx);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Jx written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jy, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Jy);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Jy written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Jz, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Jz);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Jz written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      if (nPE == 0) {
         // Creating the dataset within the dataspace                      */
         fdataset = H5Dcreate(file, Dens, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
         assert (fdataset >= 0);
         status = H5Dclose(fdataset);
         assert (status >= 0);
      }
      fdataset = H5Dopen(file, Dens);
      assert (fdataset >= 0);
      fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
      dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
      status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: Dens written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;
      ...
      for (int isort=0; isort<nsorts; isort++) {
         ...
         if (nPE == 0) {
            // Creating the dataset within the dataspace                      */
            fdataset = H5Dcreate(file, DensSort, H5T_IEEE_F32LE, fdataspace, H5P_DEFAULT);
            assert (fdataset >= 0);
            status = H5Dclose(fdataset);
            assert (status >= 0);
         }
         fdataset = H5Dopen(file, DensSort);
         assert (fdataset >= 0);
         fdataspace1 = H5Dget_space(fdataset);    /// Get filespace handle first. 
         dummy_rank      = H5Sget_simple_extent_ndims(fdataspace1);
         status  = H5Sget_simple_extent_dims(fdataspace1, dimsf, NULL);
      ...
      wcp = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: DensArray written in " << wcp - wcm << " seconds\n";
      domain()->out_Flog.flush();
      wcm = wcp;

      // Close the datafile                                             */
      float wc1 = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: it took " << wc1-wc << " seconds to save fields! \n";
      domain()->out_Flog.flush();
   ...
   H5Tinsert(electron_tid, "x", HOFFSET(SavedParticle, f_X), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "y", HOFFSET(SavedParticle, f_Y), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "z", HOFFSET(SavedParticle, f_Z), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "px", HOFFSET(SavedParticle, f_Px), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "py", HOFFSET(SavedParticle, f_Py), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "pz", HOFFSET(SavedParticle, f_Pz), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "weight", HOFFSET(SavedParticle, f_Weight), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_tid, "q2m", HOFFSET(SavedParticle, f_Q2m), H5T_NATIVE_FLOAT);

   H5Tinsert(electron_mem_tid, "cell", HOFFSET(SavedParticle, l_Cell), H5T_NATIVE_INT);
   H5Tinsert(electron_mem_tid, "x", HOFFSET(SavedParticle, f_X), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "y", HOFFSET(SavedParticle, f_Y), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "z", HOFFSET(SavedParticle, f_Z), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "px", HOFFSET(SavedParticle, f_Px), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "py", HOFFSET(SavedParticle, f_Py), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "pz", HOFFSET(SavedParticle, f_Pz), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "weight", HOFFSET(SavedParticle, f_Weight), H5T_NATIVE_FLOAT);
   H5Tinsert(electron_mem_tid, "q2m", HOFFSET(SavedParticle, f_Q2m), H5T_NATIVE_FLOAT);
   ...
   H5Tinsert(ion_tid, "x", HOFFSET(SavedIon, f_X), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "y", HOFFSET(SavedIon, f_Y), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "z", HOFFSET(SavedIon, f_Z), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "px", HOFFSET(SavedIon, f_Px), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "py", HOFFSET(SavedIon, f_Py), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "pz", HOFFSET(SavedIon, f_Pz), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "weight", HOFFSET(SavedIon, f_Weight), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_tid, "q2m", HOFFSET(SavedIon, f_Q2m), H5T_NATIVE_FLOAT);
   ...
   H5Tinsert(ion_mem_tid, "x", HOFFSET(SavedIon, f_X), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "y", HOFFSET(SavedIon, f_Y), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "z", HOFFSET(SavedIon, f_Z), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "px", HOFFSET(SavedIon, f_Px), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "py", HOFFSET(SavedIon, f_Py), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "pz", HOFFSET(SavedIon, f_Pz), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "weight", HOFFSET(SavedIon, f_Weight), H5T_NATIVE_FLOAT);
   H5Tinsert(ion_mem_tid, "q2m", HOFFSET(SavedIon, f_Q2m), H5T_NATIVE_FLOAT);
   ...
   for (nprocessor=0; nprocessor<nPEs; nprocessor++) {
      ...
      idataset = H5Dcreate(file, idataname, H5T_STD_I64LE, idataspace, H5P_DEFAULT);
      assert (idataset >= 0);
      ...
      if (npart[0] > 0) {
         particles_dataset[0] = H5Dcreate(file, pdataname, electron_tid, particles_dataspace[0], H5P_DEFAULT);
      ...
      for (is=1; is<nsorts; is++) {
         ...
         particles_dataset[is] = H5Dcreate(file, sortname, ion_tid, particles_dataspace[is], H5P_DEFAULT);
         assert(particles_dataspace[is] >= 0);
      }
         ...
         for (j=0; j<l_My; j++) {
            ...
            for (i=0; i<l_Mx; i++) {
               ...
               while(p) {
                  ...
                  status = H5Sselect_hyperslab(particles_dataspace[is], H5S_SELECT_SET, offset, NULL, count, NULL);
                  assert (status >= 0);
                  offset[0] = 0;
                  status = H5Sselect_hyperslab(particles_mem_dataspace[is], H5S_SELECT_SET, offset, NULL, count, NULL);
                  assert (status >= 0);
                  ...
                  if (is==0) {
					 ...
					 e.f_Y *= hy;
					 e.f_Z *= hz;
                     status = H5Dwrite(particles_dataset[is], electron_tid, particles_mem_dataspace[is], 
                        particles_dataspace[is], H5P_DEFAULT, &e);
                     assert (status >= 0);
                  } else {
                     SavedIon ion;
                     ion = *p;
					 ...
					 ion.f_Y *= hy;
					 ion.f_Z *= hz;
                     status = H5Dwrite(particles_dataset[is], ion_tid, particles_mem_dataspace[is], 
      ...
      float wc1 = domain()->GetCntrl()->GetWallClockElapsed();
      domain()->out_Flog << "SAVE: it took " << wc1-wc << " seconds to save particles! \n";
      domain()->out_Flog.flush();
   }
   for (is=0; is<nsorts; is++) {
      if (npart[is] < 1) continue;
