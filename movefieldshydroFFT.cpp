...
//   I.im = 1.;
   ...
   if (FirstCall == 0) {
      ...
      planC2R_Jx = fftw_plan_dft_c2r_2d(nz, ny, (fftw_complex*)vcJx, rJx, FFTW_ESTIMATE);
      planC2R_Jy = fftw_plan_dft_c2r_2d(nz, ny, (fftw_complex*)vcJy, rJy, FFTW_ESTIMATE);
      planC2R_Jz = fftw_plan_dft_c2r_2d(nz, ny, (fftw_complex*)vcJz, rJz, FFTW_ESTIMATE);
      planC2R_Rho  = fftw_plan_dft_c2r_2d(nz, ny, (fftw_complex*)vcRho, rRho, FFTW_ESTIMATE);
   }
...
//            djby = djbz = 0.;
            ...
            vcEy[n1] = -aky/(ak2+dens)*I*(rp);
            vcEz[n1] = -akz/(ak2+dens)*I*(rp);

            vcJy[n1] = jyp + h*dens*vcEy[n1];
            vcJz[n1] = jzp + h*dens*vcEz[n1];

            vcEx[n1] = -aky/ak2*I*(vcJy[n1]) -akz/ak2*I*(vcJz[n1]);
            vcJx[n1] = jxp + dens*h*vcEx[n1];

//            vcRho[n1] = rp - jxp + vcJx[n1] - complex <double>(0.,aky*h)*vcJy[n1] - complex <double>(0.,akz*h)*vcJz[n1];
...
//            vcRho[n1] = rp - jxp + vcJx[n1] - complex <double>(0.,aky*h)*vcJy[n1] - complex <double>(0.,akz*h)*vcJz[n1];
            vcRho[n1] = rp + complex <double>(h*(ak2+dens),0.)*vcEx[n1] - h*I*aky*vcJyBeam[n1] - h*I*akz*vcJzBeam[n1] ;
